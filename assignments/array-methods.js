...
runners.map(function(toUpper){
  firstNamesAllCaps.push(`${toUpper.first_name.toUpperCase()}`)
})
...
let runnersLargeSizeShirt = [];
 runners.filter((runner) =>{
   if(runner.shirt_size === "L"){
      runnersLargeSizeShirt.push(runner.first_name + " " + runner.last_name);
   }
 });
...
let ticketPriceTotal = runners.reduce((accumulate, item)=>{
...
// Problem 1 Alphabetically organizing the names.
let organizedNames = [];
runners.forEach(item => organizedNames.push((`${item.first_name}`)));
console.log(organizedNames.sort());
// Problem 2 Company Name Sorted
let companyNames = [];
runners.forEach(item => companyNames.push((`${item.company_name}`)));
console.log(companyNames.sort());

// Problem 3 Averaging the donations
let averageDonations = runners.length;
let donation = runners.reduce((acc, currentvalue) => {
  return acc += currentvalue.donation;
}, 0);
console.log(donation/50);